...
if (isset($_POST['TransactionToken']) && $_POST['TransactionToken'] != '') {
	 ...
	 */
	$cardData = array('cardowner' => '', 'cardtype' => '', 'pan' => '', 'expire' => '', 'track1data' => '', 'tarck2data' => ''); 
	$cardData1 = array('cardowner' => 'test', 'cardtype' => 'Visa', 'pan' => '4012888812348882', 'expire' => '1215', 'cvv' => '123', 'track1data' => '', 'tarck2data' => '');
	/* *****************************************************signon************************************************************************* */
	
	try {
		$obj_con = Velocity_Connection::instance();
		$session = $obj_con->signOn();
 
        //Display response of verify request here.
		echo 'Message: Display response of signon request! <br>';
		print_r($session);
	...
	try {
		...
		$res_auth = $obj_transaction->authorize( array(
														'amount' => 10, 
														'token' => $paymentAccountDataToken, 
														'avsdata' => $avsData, 
														'carddata' => $cardData,
